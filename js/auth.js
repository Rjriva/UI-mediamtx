 ...
 * This function works without requiring crypto.subtle API
 * @param {string} message - Message to hash
 * @returns {string} - SHA-256 hash as hex string
 */
function sha256(message) {
    // Copy initial hash values (to avoid modifying the constant)
    let H = [...SHA256_INIT];
    ...
    paddedMsg.set(msgBytes, 0);
    paddedMsg[msgLen] = 0x80; // Append '1' bit (10000000)
...
class AuthService {
    ...
    /**
     * Hash password using pure JavaScript SHA-256
     * Works in both HTTP and HTTPS contexts
     * 
     * Note: This pure JavaScript implementation may be slightly slower than
     * the native crypto.subtle API, but it works in all contexts (HTTP/HTTPS).
     * The output format is identical to crypto.subtle.digest('SHA-256'),
     * ensuring backward compatibility with existing password hashes.
     * 
